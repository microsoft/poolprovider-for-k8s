...
const controllerName = "finalizer_azurepipelinepool"
var log = logf.Log.WithName("controller_azurepipelinepool")

// Add creates a new AzurePipelinePool Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileAzurePipelinePool{client: mgr.GetClient(), scheme: mgr.GetScheme()}
}

// add adds a new Controller to mgr with r as the reconcile.Reconciler
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	// Create a new controller
	c, err := controller.New("azurepipelinepool-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource AzurePipelinePool
	err = c.Watch(&source.Kind{Type: &devv1alpha1.AzurePipelinePool{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}

	// TODO(user): Modify this to be the types you create that are owned by the primary resource
	// Watch for changes to secondary resource Pods and requeue the owner AzurePipelinePool
	err = c.Watch(&source.Kind{Type: &corev1.Pod{}}, &handler.EnqueueRequestForOwner{
		IsController: true,
		OwnerType:    &devv1alpha1.AzurePipelinePool{},
	})
	if err != nil {
		return err
	}
...
// blank assignment to verify that ReconcileAzurePipelinePool implements reconcile.Reconciler
var _ reconcile.Reconciler = &ReconcileAzurePipelinePool{}

// ReconcileAzurePipelinePool reconciles a AzurePipelinePool object
type ReconcileAzurePipelinePool struct {
...
func (r *ReconcileAzurePipelinePool) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	reqLogger := log.WithValues("Request.Namespace", request.Namespace, "Request.Name", request.Name)
	reqLogger.Info("Reconciling AzurePipelinePool")

	// Fetch the AzurePipelinePool instance
	instance := &devv1alpha1.AzurePipelinePool{}
...
func IsInitialized(obj metav1.Object) bool {
	azurepipelinepoolobj, ok := obj.(*devv1alpha1.AzurePipelinePool)
	if !ok {
		return false
	}
...
func manageCleanUpLogic(cr *devv1alpha1.AzurePipelinePool) error{
	// perform additional cleanup here
	return nil
}

func newPodForCR(cr *devv1alpha1.AzurePipelinePool) *corev1.Pod {
...
func newBuildkitPodForCR(cr *devv1alpha1.AzurePipelinePool) *appsv1.StatefulSet {
	labels := map[string]string {
		"app": cr.Name,
	}
...
func newBuildkitServiceForCR(cr *devv1alpha1.AzurePipelinePool) *corev1.Service {
       labels := map[string]string {
		"app": cr.Name,
	}
...
func newConfigMapForCR(cr *devv1alpha1.AzurePipelinePool) *corev1.ConfigMap {
...
func newServiceForCR(cr *devv1alpha1.AzurePipelinePool) *corev1.Service {
    labels := map[string]string {
		"app": cr.Name,
		"tier":"frontend",
	}
